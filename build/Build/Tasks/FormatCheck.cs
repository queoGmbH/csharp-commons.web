...
namespace Build
{
    ...
    public class FormatCheck : FrostingTask<Context>
    {
        ...
        public override void Run(Context context)
        {
            ...
            context.DotNetTool(
                "format",
                new DotNetToolSettings { ArgumentCustomization = args => args.Append(context.ProjectSpecifics.SolutionName).Append("--verify-no-changes") });
        ...
        {
            context.Error(
                $"Your code does not align with our code style conventions! Please run 'dotnet format whitespace --folder' and/or `dotnet format {context.ProjectSpecifics.SolutionName}` while executing this from src-Folder locally to format the code! You might need to install the dotnet-format tool on your machine with the command 'dotnet tool install -g dotnet-format'.");
            throw exception;
        }
    }
}
